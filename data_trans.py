...
def process():
    print("数据预处理")
    all_50_schemas = set()
    with open(all_50_schemas_path) as f:
        for l in tqdm(f):
            a = json.loads(l)
    ...
    with codecs.open(all_50_schemas_me_path, 'w', encoding='utf-8') as f:
    ...
    with codecs.open(train_data_me_path, 'w', encoding='utf-8') as f:
    ...
    with open(dev_data_path) as f:
        for l in tqdm(f):
            ...
            for c in a['text']:
                chars[c] = chars.get(c, 0) + 1
    with codecs.open(dev_data_me_path, 'w', encoding='utf-8') as f:
        json.dump(dev_data, f, indent=4, ensure_ascii=False)
    with codecs.open(all_chars_me_path, 'w', encoding='utf-8') as f:
